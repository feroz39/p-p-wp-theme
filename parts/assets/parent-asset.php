<?php
    ...
    $other_address       = $args['other_address'] ?? '';
    $investmentstatus    = $args['investmentstatus'] ?? '';
    $toplinks            = $args['toplinks'] ?? '';
    $content             = $args['content'] ?? '';
    $keystats            = $args['keystats'] ?? '';
    $components          = $args['components'] ?? '';
    $sections            = $args['section'] ?? '';
    $performance_content = $args['performance_content'] ?? '';
    $listing_table       = $args['listing_table'] ?? '';
    $show_read_more      = $args['show_read_more'] ?? '';
    $link_text           = $args['link_text'] ?? '';
    $cta                 = $args['cta'] ?? '';

    // echo '<pre>';
    // var_dump($listing_table);
    // echo '</pre>';
?>
